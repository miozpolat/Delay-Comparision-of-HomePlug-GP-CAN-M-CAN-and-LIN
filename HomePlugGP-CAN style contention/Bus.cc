...
void Bus::handleMessage(cMessage *msg)
{
    ...
    if(!strcmp(msg->getName(), "winnerCheck"))
    {
        int size=winners.size();
        int randomWinner=rand()%size;
        int temp=winners[randomWinner];
        cMessage *wonContention=new cMessage("wonContention");
        send(wonContention,"g$o",temp);
        ...
        for(int i=0;i<winners.size();i++)
        {
               if(i!=randomWinner)
               {
                   send(lostContention->dup(),"g$o",winners[i]);
               }
        }
        //cancelAndDelete(lostContention);
        winners.clear();

    }
    else if( isHeapAvailable==true &&!strcmp(msg->getName(), "busCheck"))
   {
   ...
   else if(!strcmp(msg->getName(),"bit"))
   {
      ...
      if(temp->getBitIndex()==(identifierLength-1))
      {
          ...
          int  abc = msg->getArrivalGate()->getIndex();
          winners.push_back(abc);
          if(winners.size()==1)
            {
              cMessage *winnerCheck=new cMessage("winnerCheck");
              scheduleAt(simTime()+0.000000001,winnerCheck);
          }
      }
   }
